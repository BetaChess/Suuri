...
inline void suuriPowWrap()
{
	...
	dontOpt = suuri::pow<testT>(a[count], b[count]);
}

inline void suuriConstWrap()
{
	BENCHMARK_START(suuri_const_benchmark);
	dontOpt = suuri::pow<7, testT>(b[count]);
}

inline void suuriPoulWrap()
{
	BENCHMARK_START(suuri_poul_benchmark);
	dontOpt = suuri::pow<uint64_t>(a[count], b[count]);
}

std::vector<void(*)()> funcList = { powWrap, suuriPowWrap, suuriConstWrap, suuriPoulWrap };
